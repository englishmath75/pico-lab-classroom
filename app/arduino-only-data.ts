    ...
    activities: [
      {
        ...
        mission: ["변수 l에 3을 저장한다.", "3번 핀을 출력으로 준비한다.", "LED를 1초 켜고 1초 끄는 동작을 반복한다.", "성공하면 시간을 0.2초로 바꾸어 비교한다."],
        functions: [{name:"pinMode(l, OUTPUT)",meaning:"3번 핀을 출력으로 준비"},{name:"digitalWrite(l, HIGH/LOW)",meaning:"LED 켜기/끄기"},{name:"delay(1000)",meaning:"1초 기다리기"}],
      ...
      },
      {
        ...
        skeleton: `int l=3;\nchar c;\n\nvoid setup(){\n  // LED 출력, 시리얼 9600\n}\n\nvoid loop(){\n  if( /* 데이터가 있으면 */ ){\n    // 문자 한 개 읽기\n    // A이면 켜기, B이면 끄기\n  }\n}`,
        answer: `int l=3;\nchar c;\n\nvoid setup(){\n  pinMode(l,OUTPUT);\n  Serial.begin(9600);\n}\n\nvoid loop(){\n  if(Serial.available()>0){\n    c=Serial.read();\n    if(c=='A') digitalWrite(l,HIGH);\n    if(c=='B') digitalWrite(l,LOW);\n  }\n}`,
      ...
      },
      {
        ...
        functions: [{name:"analogRead(A0)",meaning:"0~1023 아날로그값 읽기"},{name:"map(v,0,1023,0,255)",meaning:"입력 범위를 PWM 범위로 변환"},{name:"analogWrite(l,b)",meaning:"0~255 밝기 출력"}],
